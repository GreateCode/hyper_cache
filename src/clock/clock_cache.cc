...
inline static uint64_t RefIncr(ClockHandle *h) {
  return h->meta.FetchAddAcqRel(ClockHandle::kRefIncrement);
}

inline static void RefDecr(ClockHandle *h) {
  h->meta.FetchSubAcqRel(ClockHandle::kRefIncrement);
}
...
inline static void HitIncr(ClockHandle *h, int64_t count = 1) {
  const uint64_t old_meta =
      h->meta.FetchAddAcqRel(ClockHandle::kHitIncrement * count);
...
UniqueId64x2 DefaultHashKeyFn(const void *key, int32_t key_size,
                              uint32_t seed) {
  uint64_t h1 = 0, h2 = 0;
...
inline static bool BeginSlotInsert(const Handle &handle, ClockHandle *h,
                                   bool *already_matches) {
  assert(*already_matches == false);
  uint64_t old_meta = h->meta.FetchOrAcqRel(
...
inline static bool TryInsert(const Handle &handle, ClockHandle *h,
                             bool *already_matches) {
...
inline static bool
IsEvictionEffortExceeded(const ClockCache::EvictionData &data,
                         uint32_t eviction_effort_cap = 30) {
  return (data.freed_count + 1U) * uint64_t{eviction_effort_cap} <=
         data.seen_pinned_count;
}

inline static bool ClockUpdate(ClockHandle *h, ClockCache::EvictionData *data,
                               int64_t *hit_count_sum, int32_t hit_decr_delta) {
  ...
  if ((meta >> ClockHandle::kStateShift == ClockHandle::kStateVisible) &&
    ...
    const uint64_t new_meta =
        ...
        (new_hit_count << ClockHandle::kHitCounterShift);
    h->meta.CasStrongRelaxed(meta, new_meta);
    hit_count_sum += new_hit_count;
    return false;
  }
  if (h->meta.CasStrongAcqRel(meta, (uint64_t{ClockHandle::kStateConstruction}
                                     << ClockHandle::kStateShift) |
                                        (meta & ClockHandle::kHitBitMask))) {
    // Took ownership.
...
ClockCache::~ClockCache() {
  ...
  assert(usage_.LoadRelaxed() == 0 ||
         usage_.LoadRelaxed() == size_t{GetLength()} * sizeof(HandleImpl));
...
bool ClockCache::Insert(const Handle &handle) {
  ...
  // update
  if (already_matches && Erase(handle.hashed_key) &&
      DoInsert(handle, &already_matches)) {
    return true;
  }
      ...
      [&](HandleImpl *h) {
        ...
        if ((old_meta >> ClockHandle::kStateShift) ==
          ...
          if (h->hashed_key == hashed_key) {
            ...
            // Match. Set invisible.
            old_meta =
                h->meta.FetchAndAcqRel(~(uint64_t{ClockHandle::kStateVisibleBit}
                                         << ClockHandle::kStateShift));
...
void ClockCache::Evict(size_t requested_charge, EvictionData *data) {
  ...
  for (;;) {
    ...
    if (offset + step_size >= kDecrHitPerEntryCount) {
      hit_decr_delta =
          std::max(hit_count_sum * 1.0 / kDecrHitPerEntryCount, 1.0);
...
HandleImpl *
ClockCache::FindSlot(const UniqueId64x2 &hashed_key, const MatchFn &match_fn,
...
HandleImpl *ClockCache::DoInsert(const Handle &handle, bool *already_matches) {
  *already_matches = false;
  HandleImpl *e = FindSlot(
      handle.hashed_key,
      [&](HandleImpl *h) { return TryInsert(handle, h, already_matches); },
      [&](HandleImpl *h) {
        if (*already_matches) {
...
bool ClockCache::EvictForCharge(size_t total_charge,
...
void ClockCache::TrackAndReleaseEvictedEntry(ClockHandle *h) {
  FreeDataMarkEmpty(h);
}
