...
bool Cache::Erase(const UniqueId64x2 &hashed_key) {
  return GetShard(hashed_key)->Erase(hashed_key);
}
...
const std::vector<std::unique_ptr<ClockCache>> &Cache::GetShards() const {
  return shards_;
}

ClockCache *Cache::GetShard(size_t shard_idx) {
  return shards_[shard_idx].get();
}
