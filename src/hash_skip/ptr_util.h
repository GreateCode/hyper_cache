...
public:
    ...
    bool operator==(const MarkVersionedPtr& m) const noexcept { return p_ == m.p_; }
    bool operator!=(const MarkVersionedPtr& m) const noexcept { return p_ != m.p_; }

    uintptr_t GetPtr() const noexcept { return p_; }
    void SetPtr(uintptr_t p) noexcept { p_ = p; }
    void* GetRawPtr() const noexcept { return reinterpret_cast<void*>(p_ & 0x00007FFFFFFFFFFF); }
    ...
    // }
    int32_t GetVersion() const noexcept { return static_cast<int32_t>((p_ >> 48) & VERSION_MASK); }
    MarkVersionedPtr ClearVersion() const noexcept { return p_ & ~(0x3FFFL << 48); }
    MarkVersionedPtr SetVersion(int64_t version) const noexcept {
        return ClearVersion().p_ | ((version & VERSION_MASK) << 48);
    }
    MarkVersionedPtr IncrVersion() const noexcept { return SetVersion(GetVersion() + 1); }
    MarkVersionedPtr DecrVersion() const noexcept { return SetVersion(GetVersion() - 1); }
