    ...
    std::shared_ptr<Handle> Insert(int64_t key, std::shared_ptr<Handle> handle) {
        ...
        while (true) {
            if (Find(key, preds, succs)) {
                Node* node                         = static_cast<Node*>(succs[0].GetRawPtr());
                std::shared_ptr<Handle> old_handle = node->UpdateHandle(std::move(handle));
                // if (old_handle) {
            ...
            for (int8_t level = 0; level <= top_level; ++level) {
                ...
                new_node->nexts[level].StoreRelaxed(MarkVersionedPtr::PackPtr(succ_raw, 0, false));
            }
...
private:
    // preds: 目标前节点
    // succs: 目标或目标后节点
    bool Find(int64_t key, Node** preds, MarkVersionedPtr* succs, bool skip_marked = false) {
        bool found = false;
        Node* pred = head_;
    ...
    bool LinkLevel(Node* pred, MarkVersionedPtr expected, Node* new_node, int8_t level, bool strong) {
        // 期望的后继应当是 succs[0] (但 expectedPacked 里可能含版本/标记)
        // 在 desired 中设置 pointer=newNode，version = expected.version +1，mark=0
        const MarkVersionedPtr desired = MarkVersionedPtr::PackPtr(new_node, expected.GetVersion() + 1, false);
        if (strong) {
