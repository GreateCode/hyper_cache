...
public:
    ...
    explicit AtomicSharedPtr(std::shared_ptr<T> value) {
        std::shared_ptr<T>* raw_ptr = value ? new std::shared_ptr<T>(std::move(value)) : nullptr;
        ptr_.StoreRelaxed(MarkVersionedPtr::PackPtr(raw_ptr, 0, false));
    }

    ~AtomicSharedPtr() {
        auto* raw_ptr = static_cast<std::shared_ptr<T>*>(ptr_.LoadAcquire().GetRawPtr());
        if (raw_ptr) {
            delete raw_ptr;
    ...
    std::shared_ptr<T> Load() {
        ...
        if (auto* raw_ptr = static_cast<std::shared_ptr<T>*>(curr_ptr.GetRawPtr()); raw_ptr) {
            value = *raw_ptr;
        }
    ...
    std::shared_ptr<T> Store(std::shared_ptr<T> new_value) {
        std::shared_ptr<T>* new_raw_ptr = new_value ? new std::shared_ptr<T>(std::move(new_value)) : nullptr;
        ...
        MarkVersionedPtr new_ptr = MarkVersionedPtr::PackPtr(new_raw_ptr, 0, false);  // unmark
        ptr_.StoreRelease(new_ptr);

        std::shared_ptr<T> old_value{nullptr};
        if (auto* old_raw_ptr = static_cast<std::shared_ptr<T>*>(old_ptr.GetRawPtr()); old_raw_ptr) {
