...
public:
    HashSkipCache(int64_t capicity, int32_t bucket_num)
        : capicity_(capicity), buckets_(bucket_num), buckets_gen_(buckets_.size()), low_queue_(10000) {
    ...
    void PrintBucketsStatus() {
        for (int32_t i = 0; i < buckets_.size(); i++) {
            ...
            std::cout << "bucket:" << i << ", size:" << bucket->GetNodeNum()
                      << ", marked_deleted_num:" << bucket->GetMarkedDeletedNum() << std::endl;
        }
    }
...
private:
    ...
    void Compact() {
        ...
        while (!stop_.LoadRelaxed()) {
            ...
            std::shared_ptr<AtomicSkipList> new_bucket = CompactOneBucket(bucket_index);
            if (new_bucket) {
    ...
    std::shared_ptr<AtomicSkipList> CompactOneBucket(int32_t bucket_index) {
        std::shared_ptr<AtomicSkipList> bucket = buckets_[bucket_index].Load();
        if (bucket == nullptr || (bucket->GetMarkedDeletedNum() * 1.0 / bucket->GetNodeNum() < 0.3)) {
            return nullptr;
        }
        std::shared_ptr<AtomicSkipList> new_bucket = std::make_shared<AtomicSkipList>();
