...
public:
  ...
  int32_t MarkDeleted() {
    ...
    while (true) {
      ...
      const MarkVersionedPtr desired = MarkVersionedPtr::PackPtr(
          expected.GetRawPtr(), expected.GetVersion() + 1, true);
  ...
  std::shared_ptr<Handle> UpdateHandle(std::shared_ptr<Handle> h) {
    return handle_.Store(std::move(h));
  }
...
private:
  AtomicSharedPtr<Handle> handle_{
      nullptr}; // std::shared_ptr<Handle> handle{nullptr};
  AcqRelAtomic<int16_t> hit_count_{
      200}; // access count, initial value to avoid being immediately put into
