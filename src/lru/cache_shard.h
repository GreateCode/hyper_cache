...
template <typename Key> class LRUCacheShard {
public:
  explicit LRUCacheShard(const CacheSharedOptions &options)
...
private:
  ...
  std::unordered_map<Key, typename std::list<std::pair<Key, Handle>>::iterator>
      map_;
  std::mutex mutex_;
};
